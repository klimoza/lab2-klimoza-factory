...
mod tests {
    ...
    const MINT_WITH_DATE_STORAGE_COST: u128 = 6410000000000000000000;
    ...
    fn test_mint_with_expiration_date() {
        ...
        let token = contract.nft_mint(token_id.clone(), accounts(0), sample_token_metadata(), Some(String::from("2s")));
        assert!(token.expiration_date.is_some(), "Expiration date shouldn't be None!");
        assert_eq!(expiration_time as i64, token.expiration_date.unwrap() as i64 / 1_000_000_000);
    ...
    fn test_mint_without_date() {
        ...
        let token_id = "0".to_string();
        let token = contract.nft_mint(token_id.clone(), accounts(0), sample_token_metadata(), None);
        assert_eq!(token.token_id, token_id);
        assert_eq!(token.owner_id.to_string(), accounts(0).to_string());
        assert_eq!(token.metadata.unwrap(), sample_token_metadata());
        assert_eq!(token.approved_account_ids.unwrap(), HashMap::new());
    ...
    fn test_get_token() {
        ...
        testing_env!(context
            ...
            .build());
        let token_id = "0".to_string();
        let token = contract.nft_mint(token_id.clone(), accounts(0), sample_token_metadata(), Some(String::from("5m")));
    ...
    fn test_transfer() {
        ...
        testing_env!(context
            ...
            .build());
        let token_id = "0".to_string();
        contract.nft_mint(token_id.clone(), accounts(0), sample_token_metadata(), Some(String::from("5m")));
